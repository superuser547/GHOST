...
19. `REFERENCES_AGE_WARNING`
    - Источник старше 5 лет помечается как **warning** (рекомендация, см. раздел 3.7).
...
- Формат имени файла (норматив):
  - `Фамилия_Группа_АббревиатураДисциплины_ТипРаботы№Номер.{ext}`
...
- Шрифт в таблице: **Times New Roman, 12 pt**.
- Выравнивание текста в таблице: **по ширине**.
- Межстрочный интервал в таблице: **1,5**.
- Подпись к таблице:
...
- Год издания источников, как правило, **не должен превышать 5 лет**.  
  Источники старше 5 лет помечаются как **warning**, но не блокируют экспорт.
